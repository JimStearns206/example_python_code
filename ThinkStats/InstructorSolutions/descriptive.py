...
Modifications by Jim Stearns, May 2014:
- Placed print parameters inside parentheses to keep Python 3 happy.
- Modified import statements to reflect move of Downey utilities to a separate directory.
- Converted call to sort() method to use of built-in operator sorted that works on both Python 2 and 3:
    # Python 2:
    # live_lengths = pool.hist.GetDict().items()
    # live_lengths.sort()
    # Alternative, Python 2 and Python 3.
    live_lengths = sorted(pool.hist.GetDict().items())
"""

import math
from InstructorSolutions import first
from InstructorUtilities import Pmf
from InstructorUtilities import survey
from InstructorUtilities import thinkstats
